...
function DeckForm({deckName, onNameChange, onSave, onDelete}) {
	return html`
		<form onSubmit=${onSave}>
			<label>
				名称 <input type="text" value=${deckName} onInput=${(e) => onNameChange(e.target.value)} />
			</label>
			<button type="submit" class="Button">保存牌组</button>
			<button type="button" onClick=${onDelete} class="Button" danger>删除牌组</button>
		</form>
	`
}

function SelectedCards({selectedCards, onRemove}) {
	if (!selectedCards.length) return html`<p center>牌组是空的，添加一些卡牌吧！</p>`
...
export function DeckEditor({deck, onSaveDeck, onDeleteDeck}) {
	...
	function handleDeleteDeck() {
		...
		if (confirm(`确定要删除牌组"${deck.name}"吗？`)) {
			deleteDeck(deck.id)
