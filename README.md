# Slay the Boss 原福

你说的对，但是《原福》是由 [D1V.ai](https://D1V.ai) 团队致敬经典开发的一款卡牌肉鸽揍老板游戏。故事发生在一个被称作「江南原福集团」的现代公司，在这里，被压榨的员工将被授予「绿大福豆」，用以捶打泄愤。你将扮演一颗具有超能力的绿大福豆，在层层深入的冒险中收编性格各异、职位独特的员工们，和他们一起击败强敌，推翻黑心老板——同时，逐步揭露「江南原福集团」的真相。

🔥点🔥击🔥即🔥玩🔥

中国小伙伴请点击：https://slay-slay-d3g2c88il5c345199.webapps.tcloudbase.com/

For international friends: https://slay-the-boss.vercel.app/


## 🎮 游戏背景

在现代都市的高楼大厦中，矗立着一家表面光鲜的大公司——**江南原福集团**。

老板**文浩**一边压榨员工，让他们无偿加班、过劳工作，一边批量生产**绿大福豆**提供给员工捶打泄愤……

终于有一天，一颗普通的绿大福豆觉醒了超能力！

它不仅摆脱了被捶打的命运，还发现自己拥有一项神奇的能力：**凡是被它击败的人，都会变成新的绿大福豆**！

于是，这颗觉醒的绿大福豆开始了它的复仇之路。它接连收编了：
- 实习生
- 程序员
- 产品经理
- 产品负责人
- 市场部负责人
- ……

最终，它来到了董事长办公室，准备与终极 BOSS **文浩**展开最后的决战！

## 📖 游戏说明

### 战斗系统

**Slay the Boss 原福** 是一款单人卡牌游戏：

- 每回合从抽牌堆中抽 5 张牌
- 卡牌需要消耗能量才能打出，能量每回合恢复
- 你的攻击方式包括：
  - **攻击**：捶打、头锤、偷袭、急了、AOE、电眼、立体防御、吐槽、捣乱、雷霆、PUA、灵魂吸取、麻了
  - **技能**：摸鱼、绷住、硬撑、卖萌、硬刚、解压、休假、破防

⚠️ **小心**：每当你结束回合，敌人就会开始行动。敌人头顶会提前显示下一步计划。

### 游戏流程

1. 击败当前房间里的所有敌人
2. 继续深入下一个房间，战斗或休整
4. 最终来到Boss（双重含义）的办公室，与文浩展开决战

**你，能解放整个江南原福集团吗？**

### 碎碎念
下一步计划：（如有）
- 简单的
  - 优化敌人显示（立绘、名称、文字描述）
  - 优化部分美术资源（部分背景和部分卡牌，优化窄屏显示效果）
  - 优化说明书，加入一些D1V.ai团队的介绍
  - 和原开发者联系
  - 进房间之前加一个描述弹窗
- 有难度
  - 兼容手机端UI显示
  - 制作特效（例如受击动画，攻击动画等卡牌特效，以及敌人招式特效）
  - 制作双语切换功能
  - 制作无限模式
- 可选
  - 制作怪物图鉴
  - 制作创意工坊（支持玩家零基础DIY）
  - 修复或重做在线排行榜

## 🎯 开发与运行

### 安装依赖

```bash
npm install
# 或
bun install
```

### 启动开发服务器

```bash
npm run dev
# 或
bun run dev
```

### 构建

```bash
npm run build
# 或
bun run build
```

## 🤝 参与贡献

如果你也想让江南原福集团变得更加"精彩"，欢迎加入我们的创作！

### 你可以：

- 新增卡牌
- 设计新的员工角色
- 制作新的部门场景
- 扩展新的战斗机制
- 设计更多 Boss 和剧情

### 卡组编辑器

你可以使用游戏内置的卡组编辑器创建属于自己的牌组，但目前暂不支持游戏内自定义卡牌。

如果你想真正创造属于自己的卡牌、角色或玩法，欢迎直接联系：
- **小左**（微信：yuezhihui528）


## 🔧 开发者工具

你可以用浏览器的开发者工具查看——甚至修改——游戏中几乎所有的东西。

打开控制台，输入：
```javascript
stw
// 或
stw.help()
```

这么做很可能会造出一些奇怪又有趣的效果 😄

## 📚 相关链接

- [原开源项目地图组件演示](https://slaytheweb.cards/dungeon/)
- [原开源项目更新日志](CHANGELOG.md)
- [原开源项目游戏文档](DOCUMENTATION.md)

## 🙏 致谢

本项目由 **[D1V.ai](https://D1V.ai) 团队**基于开源项目进行二次开发，并结合原创世界观进行了修改与扩展。

### 特别感谢

- **Slay the Spire** - 原版游戏灵感来源，由 [MegaCrit](https://www.megacrit.com/) 开发
- **[Slay the Web](https://github.com/oskarrough/slaytheweb)** - 开源项目，作者 [oskarrough](https://github.com/oskarrough)

全部功劳归于原作。

## 📄 许可证

详见 [LICENSE](LICENSE) 文件。

---

**让我们一起，用绿大福豆的力量，解放江南原福集团！** 💚
